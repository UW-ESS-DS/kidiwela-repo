...
env$schema <- json$resources %>%
  lapply(function(resource) {
    ...
    temp$schema %<>%
      extract(intersect(c('primaryKey', 'uniqueKeys', 'foreignKeys'), names(.)))
    if ('primaryKey' %in% names(temp$schema)) {
      temp$schema$primaryKey %<>%
        format_element(backtick = TRUE, array = TRUE)
    }
    for (i in seq_along(temp$schema$uniqueKeys)) {
      temp$schema$uniqueKeys[[i]] %<>%
        format_element(backtick = TRUE, array = TRUE)
    }
    ...
    r$fields <- resource$schema$fields %>%
      lapply(function(field) {
        ...
        if (!is.null(temp$constraints$pattern)) {
          # '*' in patterns mess up syntax highlighting
          temp$constraints$pattern %<>% format_element(backtick = TRUE)
        }
